      ...
      <slot name="top">
        <div class="loadmore-top" v-if="topMethod">
          <div v-if="topStatus !== 'loading'" :class="topStatus === 'drop' ? 'drop' : 'pull'">
                <i v-if="loadMoreIconVisible" :class="['fa',topStatus === 'drop'? 'fa-arrow-down':'fa-arrow-up']"></i>
  ...
  export default {
    ...
    methods: {
      async onloadMoreScroll(){
          ...
          try {
            if(response.data.length <=0 /*|| response.total <= (this.param.page*this.param.per_page)*/){
                this.AllLoaded = true;
